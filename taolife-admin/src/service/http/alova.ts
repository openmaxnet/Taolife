...
import { handleBusinessError, handleRefreshToken, handleResponseError, handleServiceResult } from './handle'
import { encryptRequest, decryptResponse } from './crypto'

// 存储当前请求的 AES 密钥，用于响应解密
const requestAesKeyMap = new WeakMap<any, Uint8Array>()
...
const { onAuthRequired, onResponseRefreshToken } = createServerTokenAuthentication<VueHookType>({
  ...
  // 添加token到请求头
  assignToken: (method) => {
    method.config.headers.Authorization = `Bearer ${local.get('tf_access_token')}`
  },
})

// docs path of alova.js https://alova.js.org/
export function createAlovaInstance(alovaConfig: Service.AlovaConfig, backendConfig?: Service.BackendConfig) {
  ...
  return createAlova({
    ...
    beforeRequest: onAuthRequired(async (method) => {
      ...
      // 加密请求体
      const bodyStr = method.data ? (typeof method.data === 'string' ? method.data : JSON.stringify(method.data)) : ''
      const url = new URL(method.url, 'http://localhost')
      const path = url.pathname
      const { headers, body: encryptedBody, aesKey } = await encryptRequest(method.type, path, bodyStr)
      if (aesKey.length > 0) {
        Object.assign(method.config.headers, headers)
        method.data = encryptedBody
        requestAesKeyMap.set(method, aesKey)
      }
    ...
    responded: onResponseRefreshToken({
      ...
      onSuccess: async (response, method) => {
        ...
        if (status === 200) {
          ...
          // 解密加密响应
          const aesKey = requestAesKeyMap.get(method)
          if (apiData?.encrypted && aesKey) {
            apiData = await decryptResponse(apiData, aesKey)
            requestAesKeyMap.delete(method)
          }
          // 请求成功
          if (apiData[_backendConfig.codeKey] === _backendConfig.successCode) return handleServiceResult(apiData)
