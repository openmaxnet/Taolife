...
import { importQuestionRules } from '@/service/api/ai'
import { NAlert, NUpload, NSpace, NSpin, NUploadDragger } from 'naive-ui'
...
async function handleUpload({ file }: { file: any }) {
  ...
  result.value = null
  try {
    const res = await importQuestionRules(file)
...
<template>
  <n-modal v-model:show="modalVisible" :mask-closable="false" preset="card" title="批量导入规则" style="width: 500px">
    <NSpin :show="uploading">
      <n-upload :max="1" accept=".xlsx,.xls" :custom-request="handleUpload" :show-file-list="false">
        <NUploadDragger>
          <NSpace vertical align="center" :size="20">
            <icon-park-outline-upload-one style="font-size: 48px; color: #999" />
            <div>点击或拖拽上传 Excel 文件</div>
            <div style="font-size: 12px; color: #999">支持 .xlsx, .xls 格式</div>
          </NSpace>
        </NUploadDragger>
      </n-upload>
      ...
      <div v-if="result" style="margin-top: 20px">
        <NAlert type="success" title="导入结果" style="margin-bottom: 12px">
          <NSpace vertical>
            ...
            <div v-if="result.failCount > 0">失败：{{ result.failCount }}</div>
          </NSpace>
        </NAlert>
        <div v-if="result.failList.length > 0" style="max-height: 200px; overflow-y: auto">
          <div v-for="(item, index) in result.failList" :key="index" style="font-size: 12px; color: #999">{{ item }}</div>
        </div>
      </div>
    </NSpin>
    <template #footer>
      <NSpace justify="end"><NButton @click="closeModal">关闭</NButton></NSpace>
    </template>
  </n-modal>
</template>
